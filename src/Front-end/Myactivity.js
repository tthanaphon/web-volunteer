import {  MenuItem, Select, InputLabel, FormControl, TextField, Box, Paper, Typography, Button, Table, TableBody, TableCell, TableContainer, TableHead,
   TableRow, Grid, Dialog, DialogTitle, DialogContent, IconButton } from '@mui/material';
...
const Myactivity = () => {
  ...
  const [editMode, setEditMode] = useState(false); // สถานะแก้ไข
  ...
  const [formProfileData, setFormProfileData] = useState({
    ...
    user_img: UserData ? UserData.user_img : '',// เพิ่มรูปโปรไฟล์
  });
  ...
  const handleEditClick = () => {
    ...
    }); // คัดลอกข้อมูลเดิมจาก userData ไปยัง formProfileData
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        setFormProfileData({ ...formProfileData, user_img: reader.result });
      };
  ...
  const handleSaveProfile = () => {
    // อัปเดตข้อมูลใน UserData หลังจากกดบันทึก
    setUserData({
      ...formProfileData,
    });
  
    // สร้าง FormData เพื่อส่งข้อมูลรูปภาพและข้อมูลอื่น ๆ
    const formData = new FormData();
    formData.append('username', formProfileData.username);
    formData.append('name', formProfileData.name);
    formData.append('tel', formProfileData.tel);
    formData.append('email', formProfileData.email);
    
    // เช็คว่า user_img ไม่ว่างก่อนที่จะเพิ่มไฟล์ลงไป
    if (formProfileData.user_img instanceof File) {
      formData.append('user_img', formProfileData.user_img);
    }
      ...
      fetch(`http://127.0.0.1:8000/api/users/${storedUserId}/`)
        ...
        .then((data) => {
          ...
          setFormProfileData({
            ...
            tel: data.tel,
            email: data.email,
            user_img: data.user_img // อัปเดตโปรไฟล์ในฟอร์มด้วยข้อมูลจาก API
          });
        })
    ...
    <Grid item xs={12} md={6}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
             <Box sx={{ marginRight: 2 }}>
                  <Avatar
                    alt={formProfileData.name}
                    src={formProfileData.user_img || '/default-avatar.png'} // รูปภาพโปรไฟล์เริ่มต้น
              ...
              <Box mt={2} sx={{ marginLeft: 'auto', marginTop: 'auto', marginBottom: 0 }}>
            {editMode ? (
              ...
              <Button variant="contained" color="primary" onClick={handleEditClick}>
                <EditIcon sx={{ marginRight: 1 , width:'20px' }} />
              </Button>
            )}
          </Box>
        </Grid>
